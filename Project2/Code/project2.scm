...
;;;;;;;::;::::::::;:;::;;::::::;:;::::;;:;::;::::::::;:;::;;;::::;;;;:::;:::;;::;;;:;::;:;:;::;;;:;:::;;:;::;:;;;:::;;:
;;;    	 		     
;;; ozgurozcan15@ku.edu.tr     Tue Nov  1 11:31:03 2016
;;;    	 		     
;;; Before you start:    	 		     
;;;    	 		     
...
(define (segment-length seg)    	 		     
  (sqrt (+ (square (- (x-of (first seg)) ( x-of (second seg))))
        (square (- (y-of (first seg)) ( y-of (second seg))))))
)
...
(define (create-polygon ptlist)
  (define (helper ptlist start-pt)
    (if (= (length ptlist) 1) (list (make-seg (first ptlist) start-pt))
        (cons (make-seg (first ptlist) (second ptlist))
              (helper (cdr ptlist) start-pt)))
    )
  (helper ptlist (first ptlist))
  )
...
(define (centroid-alt lst)    	 		     
  (define (xlister lst)
    (if (null? lst) '()
        (cons (x-of (car lst)) (xlister (cdr lst)))))
  (define (ylister lst)
    (if (null? lst) '()
        (cons (y-of (car lst)) (ylister (cdr lst)))))
  (define (sum lst)
    (if (null? lst) 0
        (+ (car lst) (sum (cdr lst)))))
  (let ((x (/ (sum (xlister lst)) (length lst)))
        (y (/ (sum (ylister lst)) (length lst))))
    (make-point x y))
  )
...
;; This definition will give an error if we run it, because the procedure
;; "square" expects a number as an input, however it gets a procedure as
;; an input. The procedure "cos" should get an argument, so it will return
;; a number value which will be the input for the procedure "square".
...
;; The input of the procedure "square-a-procedure" should be a function which
;; returns a number. However the procedure deriv returns a new procedure,
;; not a number.
...
;; For n=27 they will return the same value. The inner "thrice" composes f
;; three times and the outer "thrice" composes third time composed f's
;; three times to each other (3*3*3=27).
...
(define (diagonal-line pt len)
   (lambda (t) (make-point (+ (x-of pt) (* t (/ len (sqrt 2))))
                           (+ (y-of pt) (* t (/ len (sqrt 2)))))))    	 		     
...
((diagonal-line p2 (* 3 (sqrt 2))) 1)
;;(0 4) + (3 3) = (3 7) => correct

((diagonal-line p3 (* 5 (sqrt 2))) 0)
;;(2 5) + (0 0) = (2 5) => correct
...
(define (upside-down curve)
  (lambda (t)    	 		      	 		     
    (make-point    	 		     
     (x-of (((translate 0 (- (y-of (curve 0))))
             curve) t))    	 		     
     (- (y-of (((translate 0 (- (y-of (curve 0))))
                curve) t))))))		     
...
(define (connect-ends curve1 curve2)
  (let* ((start-curve2 (curve2 0))
         (end-curve1 (curve1 1)))
    (connect-rigidly curve1 
                     ((translate (- (x-of end-curve1)
                                    (x-of start-curve2))
                                 (- (y-of end-curve1)
                                    (y-of start-curve2)))
                      curve2))))
...
;; There is no significant difference between draw-connected applications.

((draw-points-on g1 200) unit-circle)
((draw-points-on g2 200) alternative-unit-circle)

;; There is a significant difference between these two draw-points-on applications.
;; "unit-circle" procedure uses t as a multiplier in calculating the position
;; of points, while "alternative-unit-circle" procedure uses t^2. Both drawings
;; contain 200 points, which means t gets value from 0 to 1 by increasing 0.05.
;; In g1, the distances between two consecutive points are the same. But in g2,
;; The distance gets bigger as t increases, beacuse the difference of squares
;; gets bigger as t increases.
...
((draw-points-squeezed-to-window g1 10000) (param-gosper 5 (lambda (n) (/ pi (+ n 2)))))
((draw-points-squeezed-to-window g2 10000) (param-gosper 5 (lambda (n) (/ pi (expt 1.3 n)))))
((draw-points-squeezed-to-window g3 10000) (param-gosper 7 (lambda (n) (/ pi (expt 1.3 n)))))
...
(define (kochize level)
  (param-gosper (* level 2)
                (lambda (level)
                   (if (even? level) (/ pi 6) (- (/ pi 6))))))
...
((draw-points-squeezed-to-window g1 1000) (kochize 1) )
((draw-points-squeezed-to-window g2 1000) (kochize 2) )
((draw-points-squeezed-to-window g3 1000) (kochize 3) )
...
;; I worked alone using only the course reference materials.
